...
class ScheduleCampaignController
{
    ...
    public function __invoke(Campaign $campaign, ScheduleCampaignRequest $request)
    {
        ...
        if (! $campaign->isPending()) {
            flash()->error(__('mailcoach - Campaign :campaign could not be scheduled because it has already been sent.', ['campaign' => $campaign->name]));
        ...
        flash()->success(__('mailcoach - Campaign :campaign is scheduled for delivery.', ['campaign' => $campaign->name]));
