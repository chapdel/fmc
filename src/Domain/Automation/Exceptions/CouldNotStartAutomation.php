...
class CouldNotStartAutomation extends Exception
{
    ...
    {
        return new static("The automation with id `{$automation->id}` can't be started, because it has no trigger.");
    }

    public static function noListSet(Automation $automation): self
    {
        return new static("The automation with id `{$automation->id}` can't be started, because there is no list set to send it to.");
    }
}
