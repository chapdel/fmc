...
it('will automatically add html tags', function () {
    ...
    assertMatchesHtmlSnapshot($campaign->contentItem->webview_html);
});
...
it('will not double encode ampersands', function () {
    ...
    expect($campaign->contentItem->webview_html)->toContain('-&gt;');
});
...
it('will add utm tags', function () {
    ...
    expect($campaign->contentItem->webview_html)->toContain(htmlspecialchars('https://spatie.be?utm_source=newsletter&utm_medium=email&utm_campaign=my-campaign'));
    assertMatchesHtmlSnapshot($campaign->contentItem->webview_html);
});
...
it('will add utm tags to links that already have query parameters', function () {
    ...
    expect($campaign->contentItem->webview_html)->toContain(htmlspecialchars('https://spatie.be?foo=bar&utm_source=newsletter&utm_medium=email&utm_campaign=my-campaign'));
    assertMatchesHtmlSnapshot($campaign->contentItem->webview_html);
});
...
it('will add utm tags to urls with paths correctly', function () {
    ...
    expect($campaign->contentItem->webview_html)->toContain(htmlspecialchars('https://freek.dev/1234-my-blogpost?utm_source=newsletter&utm_medium=email&utm_campaign=my-campaign'));
    assertMatchesHtmlSnapshot($campaign->contentItem->webview_html);
});
...
it('will add utm tags to urls with paths correctly when the link is added twice', function () {
    ...
    expect($campaign->contentItem->webview_html)->toContain(htmlspecialchars('https://freek.dev?utm_source=newsletter&utm_medium=email&utm_campaign=my-campaign'));
    expect($campaign->contentItem->webview_html)->toContain(htmlspecialchars('https://freek.dev/1234-my-blogpost?utm_source=newsletter&utm_medium=email&utm_campaign=my-campaign'));
    assertMatchesHtmlSnapshot($campaign->contentItem->webview_html);
});
