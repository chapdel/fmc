...
it('returns false when the subscriber is already in the automation and repeat is disabled', function () {
...
it('returns true when the subscriber is already in the automation and repeat is enabled', function () {
    /** @var Automation $automation */
    $automation = Automation::create();
    $automation->name('New year!')
        ->runEvery(CarbonInterval::minute())
        ->repeat(repeatEnabled: true, onlyAfterHalt: false)
        ->to(test()->emailList)
        ->triggerOn(new SubscribedTrigger())
        ->chain([
            new SendAutomationMailAction(test()->automationMail),
...
it('returns true when the subscriber is already in the automation and repeat is enabled with check on halt', function () {
    /** @var Automation $automation */
    $automation = Automation::create();
    $automation->name('New year!')
        ->runEvery(CarbonInterval::minute())
        ->repeat(repeatEnabled: true, onlyAfterHalt: true)
        ->to(test()->emailList)
        ->triggerOn(new SubscribedTrigger())
        ->chain([
            new SendAutomationMailAction(test()->automationMail),
    ...
    ActionSubscriber::first()->update(['halted_at' => now()]);
