...
- If your `composer.json` contains any of the additional Mailcoach packages like `spatie/laravel-mailcoach-ses-feedback`, you can remove this as they are now included within the core `laravel-mailcoach` package.
- Update the requirement of `spatie/laravel-mailcoach` to `^7.0`
- `laravel/sanctum` is no longer a requirement for Mailcoach. If you are not using this inside your application, make sure `config/sanctum.php` is removed.
