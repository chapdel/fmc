<x-mailcoach::layout-main :title="__('mailcoach - Transactional log')">
    <div class="table-actions">
        @if($transactionalMailsCount)
            <div class="table-filters">
                <x-mailcoach::search :placeholder="__('mailcoach - Filter transactional mails…')"/>
            </div>
        @endif
    </div>
    ...
    @if($transactionalMailsCount)
        <table class="table table-fixed">
            <thead>
            <tr>
                <x-mailcoach::th sort-by="subject">{{ __('mailcoach - Subject') }}</x-mailcoach::th>
                <x-mailcoach::th>{{ __('mailcoach - To') }}</x-mailcoach::th>
                ...
                <x-mailcoach::th sort-by="-created_at" sort-default class="w-48 th-numeric hidden | xl:table-cell">{{ __('mailcoach - Sent') }}
                </x-mailcoach::th>
                            ...
                            <ul>
                                <li>
                                    <x-mailcoach::form-button
                                        ...
                                        method="DELETE"
                                        data-confirm="true"
                                        :data-confirm-text="__('mailcoach - Are you sure you want to delete this transactional mail from the log?')"
                                    >
        ...
        <x-mailcoach::table-status :name="__('mailcoach - mail|mails')" :paginator="$transactionalMails" :total-count="$transactionalMailsCount"
                                    :show-all-url="route('mailcoach.transactionalMails')"></x-mailcoach::table-status>
    @else
        <x-mailcoach::help>
            {!! __('mailcoach - No transactional mails have been sent yet!') !!}
        </x-mailcoach::help>
    @endif
