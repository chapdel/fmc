<form class="form-grid" action="{{ route('mailcoach.emailLists.store') }}" method="POST">
    ...
    <x-text-field :label="__('Reply-to email')" :placeholder="auth()->user()->email" name="default_reply_to_email" type="email" required />
    <x-text-field :label="__('Reply-to name')" :placeholder="auth()->user()->name" name="default_reply_to_name" />
