...
<div class="md:col-span-{{ $cols }} h-full {{ $attributes->get('class') }} {{ isset($link) ? 'dashboard-tile-hover' : ''}} {{ $danger ? 'dashboard-tile dashboard-tile-error' : 'dashboard-tile'}} {{ $success ? 'dashboard-tile dashboard-tile-success' : 'dashboard-tile'}}" {{ $attributes->except('class') }}>
    @if ($icon)
