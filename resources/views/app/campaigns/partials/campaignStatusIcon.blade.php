@if($status === \Spatie\Mailcoach\Domain\Campaign\Enums\CampaignStatus::Draft)
    @if($campaign->scheduled_at)
        <x-mailcoach::rounded-icon minimal size="md" title="{{ __('mailcoach - Scheduled') }}" type="warning" icon="far fa-clock"/>
    @else
        <x-mailcoach::rounded-icon minimal size="md" title="{{ __('mailcoach - Draft') }}" type="neutral" icon="far fa-pen"/>
    @endif
@elseif ($status === \Spatie\Mailcoach\Domain\Campaign\Enums\CampaignStatus::Sent)
    <x-mailcoach::rounded-icon minimal size="md" title="{{ __('mailcoach - Sent') }}" type="success" icon="far fa-check"/>
